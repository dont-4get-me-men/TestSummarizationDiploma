...
Абстракті методи - це методи, які полягають в тому, щоб витягнути головну суть з тексту в тому числі перефразовуючи його. Для того, щоб це зробити з гарною якістю нам варто звернути увагу на :
	1. Яким чином представити весь документ 
	2. Як згенерувати послідовність слів декодером

Ми надалі розглянемо декілька різних абстрактних методів

##### Bi-LSTM

Цей тип архітектури нейронної мережі включає в собі поєднання двох методів LSTM (довга коротко-строкова память ) та Biderectional procesing (Двостороння обробка). 

Для того, щоб її зрозуміти давайте розглянемо ці два концепти ближче

LSTM: 
Рекурсивна нейронна мережа, яка може контролювати ступінь забуття інформації. Складається з декількох воріт:
1. Ворота забуття. Контролюються коефіцієнтом забуття, який визначає частку забуття з попереднього чаосового кроку. Коефіцієнт від 0 (повне забуття)  до 1(повна збереження).
2. Ворота входу. Визначає частку даних, що буде додана до попереднього стану LSTM. 
3. Ворота виходу. Визначає частку інформації, яка буде виведена на поточний часовий крок.
Коєфіцієнти на кожному кроці визначається за допомогою сигмоїди.

 Двостороння обробка:
 Маючи послідовність даних в звичайних RNN ми ідемо зліва направо в послідовностях, але в нашому випадку вона іде як вперед так і назад. Оскільки вихід в цій мережі залежить як від минулого і майбутнього і обчислються як конкатинації векторів проходу зліва направо та зправа наліво. 

![[bi-lstm.png]]

##### Використання трансформерів:
Наразі трансформери дають найкращі результати в будь-яких задачах NLP. 
Структура трансформера була представлена вперше в [19]. 
![[transformer structure.png]]
При розумінні трансформерів варто звернути увагу на декілька речей:
1. Механізм самоуваги. 
		Модель за допомогою цього механізму визначає важливість кожного зі слів. Математично це обчислюється за допомогою лінійних перетворень матриць  Q(запит), K(ключа), V(значень). Скалярно перемноживши вектори і застосувавши softmax. 
2. Multi-Head attention
		 Механізм самоуваги застовується паралельно з різними параметрами. Після цього вони конкатинуються  та лінійно проектуються
3. Позиція слів:
	Оскільки трансформери не мають уявлення про те, який порядок слів в реченні, то додатково надається позиційне кодування за допомогою синусоїдальних функцій.



Зокрема для тесту в практичній частині ми використаємо Pegasus,T5 та Gemini. 



## Практична частина
Надалі ми будемо використовувати не лише результати, що ми отримали від написання коду самими, але використовуючи результати з відкритих джерел та літератури.

Для того, щоб порівняти моделі ми будемо використовувати популярний датасет The CNN / DailyMail Dataset  - датасет з більш ніж 300 000 унікальних статей та їх вижимкою , які написані журналістами CNN та Daily Mail.

Для порівняння я використав декілька різних алгоритмів зокрема:
Text Rank ( файл text-lex-rank)
Трансформер Pegasus (pegasus)
Натренований та чистий T5 (T5 summarization)
Bi-LSTM(Bi-LSTM)
...
[18] Understanding Bidirectional LSTM for Sequential Data Processing, https://medium.com/@anishnama20/understanding-bidirectional-lstm-for-sequential-data-processing-b83d6283befc
[19] Attention Is All You Need, Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N. Gomez, Lukasz Kaiser, Illia Polosukhin, 2017
[20]